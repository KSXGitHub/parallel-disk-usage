...
- When the user provides a diff and you need to update the files, apply it with `git apply` instead of manually interpreting each hunk via the editor. If the user provides a diff for context or discussion rather than as a change to apply, respond accordingly instead.
