...
- When the user provides a diff and you need to update the files, don't manually interpreting each hunk (that'd be slow), apply it with `git apply` instead. If the user provides a diff for context or discussion rather than as a change to apply, respond accordingly instead.
